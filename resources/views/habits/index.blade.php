...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div>
                <p class="text-sm text-gray-600 mb-1">Today's Progress</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900">{{ round($completionRate) }}%</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($habits as $habit)
            <div class="bg-white rounded-xl shadow-sm border {{ $habit->completed_today ? 'ring-2 ring-blue-600' : 'border-gray-200' }} hover:shadow-md transition-all p-6">
                <div class="space-y-4">
                    <!-- Habit Header -->
                    ...
                    <div class="flex items-center gap-1.5">
                        @foreach($habit->weekly_progress as $completed)
                            <div class="h-7 w-7 rounded-lg flex items-center justify-center {{ $completed ? $habit->color : 'bg-gray-200' }}">
                                @if($completed)
                    ...
                    <form action="{{ route('habits.toggle', $habit) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="w-full py-2 px-4 rounded-lg font-medium transition {{ $habit->completed_today ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-blue-600 text-white hover:bg-blue-700' }}">
        ...
        @empty
            <div class="col-span-full text-center py-12 bg-white rounded-xl border border-gray-200">
                ...
                <p class="text-gray-600">No habits yet. Create your first habit to get started!</p>
            </div>
        @endforelse
    </div>
</div>
...
<div id="habitModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-xl max-w-md w-full p-6">
        <h2 class="text-xl font-bold mb-4">Add New Habit</h2>
        <form action="{{ route('habits.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" required placeholder="e.g., Morning Run" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            <div>
                ...
                <div class="grid grid-cols-5 gap-2">
                    @foreach(\App\Models\Habit::getColors() as $color)
                        <label class="cursor-pointer">
                            ...
                            <div class="h-10 w-10 rounded-lg {{ $color }} peer-checked:ring-2 peer-checked:ring-blue-600 peer-checked:ring-offset-2 transition-all"></div>
                        </label>
                    @endforeach
                </div>
            </div>
